...
export default function NodeProperties({
  ...
  // Whether a drill can name a predicate at all. See renderDrillButton.
  ...
  // The drill is offered against the SOURCE PREDICATE — so we have to prove the
  // key IS one.
  //
  // This panel is NOT inherently safe just because it looks structured. The
  // graph parser stores the RAW response key (lib/graph.js: `properties.attrs
  // [prop] = val`), so `{ q(func: has(name)) { uid title: name } }` puts
  // attrs.title = 'Alice' here, identical in shape to a real predicate. Both
  // `title` and `name` can be real, indexed predicates, so neither the key nor
  // the schema can tell them apart — a drill would emit eq(title, "Alice") for
  // a value that came from `name`. Only the query that produced the response
  // knows, which is why it has to be plumbed all the way down to this button.
  //
  // Node EXPANSIONS also feed this panel, but they are generated by
  // FrameSession as bare `expand(_all_)` with no aliases, so gating on the
  // frame's own query stays sound for them.
  const renderDrillButton = (key, value) => {
    ...
    if (!drillable || !isEditable(value) || key === 'uid') {
      return null
    }
