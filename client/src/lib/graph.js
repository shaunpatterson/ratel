...
function aggregationPrefix(properties) {
  const aggTerms = ['count(', 'max(', 'min(', 'sum(']
...
export function getNodeLabel(properties, regex) {
  ...
  const nameKey = getNameKey(properties, regex)
  const val = properties[nameKey]
...
export class GraphParser {
  ...
  processQueue = (regexStr = null, maxAdd = FIRST_RENDER_LIMIT) => {
    ...
    while (this.queue.length > 0) {
      ...
      for (const prop of Object.keys(obj.node).sort()) {
      ...
      const groupProperties = this.labeler.getGroupProperties(obj.src.pred)
      ...
      if (!node) {
        ...
        node.properties = Object.assign({}, n.properties, node.properties)
